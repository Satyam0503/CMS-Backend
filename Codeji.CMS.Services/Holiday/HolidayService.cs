...
public class HolidayService : IHolidayService
{
    ...
    public async Task<Result<HolidayRequestDto>> GetAllHoliday(HolidayFilter? filter)
    {
        ...
        {
            Expression<Func<Holidays, bool>> whereCondition = h => (string.IsNullOrEmpty(filter.HolidayName) || h.HolidayName.Contains(filter.HolidayName) && (!filter.HolidayType.Any() || filter.HolidayType.Contains(h.HolidayType))
            );
            holidayList = await _holidaysRepo.GetAggregateDataAsync<Holidays>(whereCondition);
        }
